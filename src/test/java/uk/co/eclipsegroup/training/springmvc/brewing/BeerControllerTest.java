...
class BeerControllerTest {
    ...
    @Test
    void tryingToCalculateSum_resultsInError(SoftAssertions softly) throws Exception {
        var mvcResult = mockMvc.perform(get("/beers/sum"))
                .andExpect(status().is(HttpStatus.INTERNAL_SERVER_ERROR.value()))
                .andReturn();

        softly.assertThat(mvcResult.getModelAndView().getViewName()).isEqualTo("global-error");
    }
}